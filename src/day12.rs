...
impl Moon {
    ...
    pub fn delta_vel(&self, moons: &[Moon]) -> Vec3 {
        moons.iter().fold(Vec3::new(), |mut vel, m| {
            if self.pos.x > m.pos.x {
                vel.x -= 1
            } else if self.pos.x < m.pos.x {
                vel.x += 1
            };
            if self.pos.y > m.pos.y {
                vel.y -= 1
            } else if self.pos.y < m.pos.y {
                vel.y += 1
            };
            if self.pos.z > m.pos.z {
                vel.z -= 1
            } else if self.pos.z < m.pos.z {
                vel.z += 1
            };
            vel
        })
...
fn part_one(input: &[Vec3]) -> i32 {
    let mut moons: Vec<Moon> = input
        .iter()
        .map(|pos| Moon::new_from_pos(pos.clone()))
        .collect();
...
fn part_two(input: &[Vec3]) -> i64 {
    ...
    (1..).find(|x| {
        ...
        if moons.iter().all(|m| m.vel.x == 0) && x_cycle == -1 {
            x_cycle = *x
        };
        if moons.iter().all(|m| m.vel.y == 0) && y_cycle == -1 {
            y_cycle = *x
        };
        if moons.iter().all(|m| m.vel.z == 0) && z_cycle == -1 {
            z_cycle = *x
        };
